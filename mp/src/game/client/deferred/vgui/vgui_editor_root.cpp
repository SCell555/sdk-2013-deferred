...
protected:
	...
	void OnMousePressedInternal( MouseCode code );
...
void CVGUILightEditor::OnMousePressedInternal( MouseCode code )
{
	StopDrag();
...
void CVGUILightEditor::OnThink()
{
	...
	if ( ipanel()->HasParent( panel_over, panel_self ) && panel_over != panel_self )
	{
	...
	if ( !bCapturingMouse )
	{
		for ( int i = 0; i < iNumMouse; i++ )
		{
			...
			{
				if ( bMouseLeft_Cur[ i ] )
					OnMousePressedInternal( iMouse_Map[ i ] );
				else
