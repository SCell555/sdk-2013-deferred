...
bool ShaderEditorHandler::IsReady() const
{
	return m_bReady;
}
...
{
	V_memcpy( &clCallback_data.mat_View, &view, sizeof( VMatrix ) );
}
...
void ShaderEditorHandler::PrepareCallbackData()
{
	...
	if ( sun.IsValid() )
	{
		Vector dir = sun->m_vDirection;
		dir.NormalizeInPlace();

		Vector screen;

		if ( ScreenTransform( MainViewOrigin() + dir * 512, screen ) )
			ScreenTransform( ( MainViewOrigin() - dir * 512 ), screen );

		screen *= Vector( 0.5f, -0.5f, 0 );
		screen += Vector( 0.5f, 0.5f, 0 );

		Q_memcpy( clCallback_data.sun_data.Base(), screen.Base(), sizeof( float ) * 2 );
		clCallback_data.sun_data[2] = DotProduct( dir, MainViewForward() );
		clCallback_data.sun_dir = dir;

		trace_t tr;
		UTIL_TraceLine( MainViewOrigin(), MainViewOrigin() + dir * MAX_TRACE_LENGTH, MASK_SOLID, NULL, COLLISION_GROUP_DEBRIS, &tr );
